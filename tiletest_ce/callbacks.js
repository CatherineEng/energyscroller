...
// FIXME: scroll animation breaks the page layout when jump-targets are enabled
// nevermind it just always breaks the page layout no matter what??? It was working for a minute there. I'm crashing out
const JUMP_TARGETS_ENABLED = true;


// TODO: revert all this junk back to the version that had readable variable-names and formatting
  ...
  for (const tile of tiles) {
    ...
    if (tile.hasAttribute("energy")) {
      ...
      if (tile.getElementsByClassName("target").length === 0) {
        ...
        jump.innerText = txt;
        if (enableJumpTargets) tile.parentNode.insertBefore(jump, tile);
      }
...
function CreateElementButtons(enableJumpTargets) {
  ...
  if (!enableJumpTargets) { wrap.setAttribute("hidden", true); return; }
  let n = 1;
  for (const t of targets) {
...
function InitStickyDurations() {
  ...
  nodes.forEach((card) => {
    ...
    // TODO: FIXME: everything below is dubious. This entire function is dubious.
    // it breaks the page layout, but seems necessary for the scrolling animation
    // return;
...
// --- GLOBAL SCROLL THRESHOLDS --- // moved from tilecount.js
...
InitGlobalScrollThresholds(); // must be called before the others
InitScrollThresholds(); // not actually necessary??
InitStickyDurations(); // must be called LAST (or the card disappears forever)
